...
declare namespace KWin {
  ...
  enum Lut1DInterpolation {
    Linear
  }

  enum Lut3DInterpolation {
    Tetrahedal = 0
  }
  ...
  class WorkspaceWrapper {
    ...
    readonly activeScreen: KWin.LogicalOutput;

    readonly screens: KWin.LogicalOutput[];

    readonly screenOrder: KWin.LogicalOutput[];
    ...
    /** @Q_INVOKABLE  */ 
    screenAt(pos: QPointF): KWin.LogicalOutput;
    /** @Q_INVOKABLE  */ 
    tilingForScreen(screenName: string): KWin.TileManager;
    /** @Q_INVOKABLE  */ 
    tilingForScreen(output: KWin.LogicalOutput): KWin.TileManager;
    /**
     * Returns the root tile for the given output and desktop.
     * @Q_INVOKABLE 
     */
    rootTile(output: KWin.LogicalOutput, desktop: KWin.VirtualDesktop): KWin.Tile;
    /**
     ...
     * @Q_SCRIPTABLE 
     */
    clientArea(option: KWin.WorkspaceWrapper.ClientAreaOption, output: KWin.LogicalOutput, desktop: KWin.VirtualDesktop): QRectF;
    /**
    ...
    /**
     * Manual patch: list of windows currently managed by KWin (scripting
     * helper added by the scripting layer, not part of the C++ Workspace API).
     */
    windowList(): KWin.Window[];
    /**
     * Manual patch: toggles the maximization state of the active window
     * (Q_SLOT of KWin::Workspace exposed to the scripting environment).
     */
  ...
  class Window {
    ...
    readonly output: KWin.LogicalOutput;
...
When an application failed to react on a ping request in time, it is considered unresponsive. This usually indicates that the application froze or crashed. */
     ...
     * @Q_INVOKABLE 
     */
    setMaximize(vertically: boolean, horizontally: boolean, restore?: RectF): void;
    ...
    bufferGeometryChanged: Signal<(oldGeometry: KWin.RectF) => void>;
    /** This signal is emitted when the Window's frame geometry changes. */
    frameGeometryChanged: Signal<(oldGeometry: KWin.RectF) => void>;
    /** This signal is emitted when the Window's client geometry has changed. */
    clientGeometryChanged: Signal<(oldGeometry: KWin.RectF) => void>;
    ...
    interactiveMoveResizeStepped: Signal<(geometry: KWin.RectF) => void>;
    ...
    excludeFromCaptureChanged: Signal<() => void>;
    decorationPolicyChanged: Signal<() => void>;
  }
...
Only to be used for effects */
  ...
  class Tile {
    /** Properties */
    relativeGeometry: KWin.RectF;

    readonly absoluteGeometry: KWin.RectF;

    readonly absoluteGeometryInScreen: KWin.RectF;
    ...
    readonly parent: KWin.Tile.LayoutDirection;
